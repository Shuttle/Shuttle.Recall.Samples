...
namespace Shuttle.TenPinBowling.Shell
{
    public class Model : IModel
    {
        ...
        public void AddFrameScore(int frame, int pins)
        {
        ...
        public void AddFrameScore(int frame, int roll, int pins)
        {
            var model = GetFrame(frame);

            if (model == null)
            {
                _frameScores.Add(new FrameScoreModel(frame, pins));
            }
            else
            {
                if (roll == 2)
                {
                    model.SetRoll2Pins(pins);
                }
                else
                {
                    model.SetRoll3Pins(pins);
                }
            }
